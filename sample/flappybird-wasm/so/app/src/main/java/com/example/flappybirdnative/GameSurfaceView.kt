package com.example.flappybirdnative
...
class GameSurfaceView @JvmOverloads constructor(
    ...
    private val native = (context as MainActivity)
    ...
    override fun surfaceCreated(holder: SurfaceHolder) {
        val canvasW = native.getCanvasWidth()
        val canvasH = native.getCanvasHeight()
    ...
    private fun drawGame(canvas: Canvas) {
        ...
        // Pipes
        val count = native.getPipeCount()
        paint.color = 0xFF73BF2E.toInt()
        for (i in 0 until count) {
            val px = native.getPipeX(i)
            val gapY = native.getPipeGapY(i)
        ...
        canvas.drawCircle(BIRD_X, native.getBirdY(), BIRD_RADIUS, paint)
    }
}
